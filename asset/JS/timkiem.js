            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                if (query) {
                    // Nếu có tham số 'q' trong URL, thực hiện tìm kiếm và hiển thị kết quả
                    ktTimKiem(query);
                }
            });

               document.addEventListener("DOMContentLoaded", function () {
                const query = localStorage.getItem("timKiemQuery");
            
                if (query) {
                    // Nếu có dữ liệu tìm kiếm trong localStorage, hiển thị kết quả tìm kiếm
                    ktTimKiem(query);
            ...
            //         // Nếu có dữ liệu tìm kiếm trong localStorage, hiển thị kết quả tìm kiếm
            //         ktTimKiem(query);
            //         localStorage.removeItem("timKiemQuery"); // Xóa dữ liệu tìm kiếm sau khi sử dụng
            //     }
            // });
            ...
            function napSach(sach){
                ...
                document.querySelector('#myTable').insertAdjacentHTML('beforeend', row);
            ...
            function ktTimKiem(query) {
                // Xóa nội dung hiện tại trong bảng
                document.getElementById("myTable").innerHTML = "";
            
                // Tìm sách theo tên (hoặc thông tin tùy loại tìm kiếm của bạn)
                const results = dsSach.filter(sach => {
                    // tìm kiếm sách dựa trên tên sách
                    return sach.ten.toLowerCase().includes(query.toLowerCase());
                });
            
                if (results.length === 0) {
                    document.getElementById("timkiem").innerHTML = "Không tìm thấy kết quả nào.";
                    return false;
                }
            
                // Hiển thị kết quả tìm kiếm
                results.forEach(napSach);
